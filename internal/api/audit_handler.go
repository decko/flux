...
// It verifies the hash chain across all audit events and returns
// whether the chain is intact. Admin role required.
func (s *Server) handleAuditIntegrity(w http.ResponseWriter, r *http.Request) {
	...
	var firstBrokenAt *string
	if result.FirstBrokenAt != nil {
		s := result.FirstBrokenAt.Format(time.RFC3339Nano)
		firstBrokenAt = &s
	}
